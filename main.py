...
Нажмите кнопку для быстрого ввода данных!
    ...
    async def templates(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        templates_message = """
            ...
            🟢 Шаблон для закупки топлива:
            5513
            Купил 200 литров по 58 грн
            [фото чека]
            
            Важно: Указываем номер авто, слово "Купил", литры и цену за литр.
            
            ---
            
            🔵 Шаблон для заправки автомобиля:
            5513
            Заправка 30 литров. Пробег: 125000 км
            [фото чека]
            
            Важно: Указать литры и пробег. Фото желательно прикрепить.
            
            ---
            
            🟡 Шаблон для заправки генератора:
            5513
            Заправка генератора
            10 литров, цена 60 грн, моточасы: 255
            [фото чека]
            
            Важно: Указать моточасы обязательно, иначе запись не будет принята.
        """
